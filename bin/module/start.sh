...
###
 ...
 # @LastEditTime: 2023-01-07 12:56:58
 # @Description: 启动服务器
...
if [ $? -eq 0 ]; then
  ...
  cmd=${cmd:-"${JvmPath:-java}"}" -server ${ExtraParameters}-Xss512K -Xmx${MaxCache}M -Xms${StartCache}M -jar ${MainJAR}.jar nogui | minecraftctl listen; exit 0;"
  # 创建一个对应名称的会话
